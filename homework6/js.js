...
const Diamond = function(str, height){
	if (height % 2 === 0) {height = height + 1}


	const Str = function(str, num){
		if (num <= 0) {return ""}

			return str + Str(str, num - 1);
	};

	const Lines = function(l, numSpaces, numStrs){
		if (l > height) {return ""}

			console.log(Str(" ", numSpaces) + Str(str, numStrs));

			if (l < (height+1)/2) {
				Lines(l+1, numSpaces - 1, numStrs + 2);
			}
			else{
				Lines(l+1, numSpaces + 1, numStrs - 2);
			}
...
const Diamonds = function(str, height){
	if (height % 2 === 0) {
		height = height + 1;
	}
	...
	for(let i = 1; i <= height; i++) {
		console.log(Str(" ", spaceN) + Str(str, charN));
		if (i < height/2 ) {
		spaceN = spaceN - 1;
		charN = charN + 2;
	}
...
const nextMove = function(board, isX)
{
	...
	return drawB(board, print);
};

const drawB = function(arr, value)
{
	var	x = rand();
	var y = rand();
...
const determineBoards = function(xLocation,yLocation)
{
	...
	for (var i = 0; i < locAndCoordinates.length; i++ )
	{
	 ...
	 {
		 alert("Game over: Restart in 1 sec");
